...
0.8 schema bump (``schema_version`` ⇒ ``"0.8"``)
-------------------------------------------------
Added fields:
...
All fields default to ``None`` so older result files (which lack them)
...
All new fields default to ``None``. Pre-1.0 result files keep loading
unchanged — the only place the bump bites is
...
# Typed enumerations for fields previously declared as ``str``.
...
class Provenance(BaseModel):
    ...
    seed: int
    # Optional additions — all default to None so older JSON loads unchanged.
    ...
    # All optional so older JSON loads unchanged; the
...
def _utc_now_iso(now: _dt.datetime | None = None) -> str:
    ...
    This honors the ``SOURCE_DATE_EPOCH`` environment variable (the
