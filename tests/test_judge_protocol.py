...
Third-party custom judges must be able to satisfy the
...
``Judge.kind`` is a runtime-enforced literal. The static
...
def test_third_party_judge_via_ajudge_responses(
    ...
    """A custom judge satisfies the Protocol structurally."""
...
def test_resolve_judge_runtime_rejects_unknown_kind() -> None:
    """A judge whose ``kind`` is not one of the documented
...
def test_resolve_judge_accepts_documented_kinds(
    ...
    """All documented ``kind`` values are accepted (the implementation: includes 'manual')."""
